...
export async function POST(req) {
  ...
  if (!String(file.type || "").startsWith("image/")) return NextResponse.json({ error: "Sirf image allowed hai" }, { status: 400 });
  if (file.size > 6 * 1024 * 1024) return NextResponse.json({ error: "Screenshot 6MB se choti honi chahiye" }, { status: 400 });
