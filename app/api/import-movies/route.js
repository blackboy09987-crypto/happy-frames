...
import { MOVIES } from "@/lib/movies-seed";

export const dynamic = "force-dynamic";

// Admin: movie frames one-time import (img se upsert)
export async function POST() {
  if (!(await isAdminRequest())) return NextResponse.json({ error: "unauthorized" }, { status: 401 });
  const supabase = getAdminClient();
  if (!supabase) return NextResponse.json({ error: "Supabase configure nahi hai" }, { status: 400 });

  const { data: existing } = await supabase.from("products").select("id,img").eq("cat", "Movies");
  const byImg = new Map((existing || []).filter((r) => r.img).map((r) => [r.img, r.id]));

  const now = Date.now();
  let added = 0, updated = 0;
  for (let i = 0; i < MOVIES.length; i++) {
    const m = { ...MOVIES[i], sort: now + i };
