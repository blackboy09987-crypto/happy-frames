...
export default function AdminPage() {
  ...
  const importMotivational = async () => {
    if (!confirm("8 motivational frames import/update karein? (Discipline, Push Your Limits, Consistency...)")) return;
    const r = await fetch("/api/import-motivational", { method: "POST" });
    const d = await r.json().catch(() => ({}));
    if (r.ok) { showToast(`Motivational: ${d.added || 0} add, ${d.updated || 0} update 💪`); loadProducts(); }
    else showToast(d.error || "Import fail");
  };
        ...
        <div className="acard">
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap", marginBottom: 6 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              <button className="btn btn--ghost btn--sm" onClick={importMotivational}>💪 Motivational (8)</button>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {orders.map((o) => (
            <div className="ordrow" key={o.id}>
              ...
              <div className="orditems">{(o.items || []).map((it, i) => (
                <span key={i}>{i > 0 ? ",  " : ""}{it.name}{it.size ? ` (${it.size})` : ""} ×{it.qty}{it.custom && it.img ? <> — <a href={it.img} target="_blank" rel="noopener" style={{ color: "var(--mint)" }}>🖼️ design</a></> : null}</span>
              ))}</div>
            </div>
          ))}
        </div>
